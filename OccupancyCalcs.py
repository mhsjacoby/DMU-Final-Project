...
occupancy probability transitions. Also plots the actual occpuancy data by weeks
When run alone this creates occupancy plots
When imported it only reads the data and creates the occupancy transitions
...
class HomeData():
    def __init__(self, path, write, h='H1', s='black'):
        self.root_dir = path
...
class HomeOccupancy(HomeData):
    ...
    def get_ground_truth(self):
        occupant_files = my.mylistdir(self.ground_path, '.csv')
        occupants = {}
    ...
    def write_occupancy_csv(self, df, fname):#, write_dir):   
        target_dir = my.make_storage_directory(os.path.join(self.write_dir, 'Occupancy_CSVs'))
...
if __name__ == '__main__':
    ...
    H1_occ.main()
    H1_occ.write_occupancy_csv(H1_occ.df_hr, f'{H1_occ.home}-{H1_occ.system}-Occupancy_df.csv')#, self.write_dir)
    ...
    H1_p1_probs.main()
    H1_p1_probs.write_occ(H1_p1_probs.Home, f'{name}-given-occupied.csv', ['hour', 'prob stay', 'prob leave'])
    H1_p1_probs.write_occ(H1_p1_probs.notHome, f'{name}-given-unoccupied.csv', ['hour', 'prob arrive', 'prob not arrive'])
