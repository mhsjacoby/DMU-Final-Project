...
# This class generates the ToTi -> Ti' transitions
...
### This class creates the To->To' transition matrices
...
def generate_ToTi(action, read_dir):                       # Given the results file from simulink, create TiTo->Ti' transitions (by action)
    t = ToTiTransitionsCSV(read_dir, action)
    t.main()
...
def combine_occ_data(h, occD, tempDF):
    store = my.make_storage_directory('/Users/maggie/Desktop/occ_inds')
    np.savetxt(os.path.join(store, f'{h}-occ-home.csv'), occD.Home[h], fmt='%s', delimiter=',')
    np.savetxt(os.path.join(store, f'{h}-occ-nothome.csv'), occD.notHome[h], fmt='%s', delimiter=',')




    home_home, home_away = occD.Home[h][1], occD.Home[h][0]         # Get probs of o'=home, o'=leave, for each hour, given o=home
    ...
    indsH = df_givenH.index
    new_indsH = []
    for i in indsH:
        new_i = i.strip(')') + ', 1)'
        new_indsH.append(new_i)
    df_givenH.index = new_indsH
    ...
    indsA = df_givenA.index
    new_indsA = []
    for i in indsA:
        new_i = i.strip(')') + ', 0)'
        new_indsA.append(new_i)
    df_givenA.index = new_indsA

    full_df = pd.concat([df_givenH, df_givenA], axis = 0)           # Create full Occ inlcuded df
    new_index = new_indsH + new_indsA
    full_df.columns = new_index
    full_df.index = new_index
    return full_df

def write_inds(inds, root_dir):
    ind_list = [f'{i},{x.strip("()")}' for i, x in enumerate(inds) ]
    ind_list = [x.split(',') for x in ind_list]
    np.savetxt(os.path.join(root_dir, 'row_ind-to-value.txt'), ind_list, fmt='%s', delimiter='\t')
    np.savetxt(os.path.join(root_dir,'row_ind-to-value.csv'), ind_list, fmt='%s', delimiter=',')
...
######################### This is the function that gets called in the main script and outputs numpy array ###########################
def create_all(root_dir):
    ...
    occupancy_path = os.path.join(root_dir, 'Occupancy')
    ...
    TT_0, L0, l0 = generate_ToTi(actions[0], root_dir)                       # Create ToTi->Ti' for each action
    TT_1, L1, l1 = generate_ToTi(actions[1], root_dir)
    TT_2, L2, l2 = generate_ToTi(actions[2], root_dir)
    ...
    for h in hours:
        ...
        for TT in action_dfs:
            ...
            # print(occ_probs.Home)

            ind_list = ToTiOcc_df.index
            write_inds(ind_list, root_dir)

            np_df = ToTiOcc_df.to_numpy()
            if np.isnan(np_df.sum()):
                num_nans += 1
                print(f'Numpy nans in action: {actions[TT[1]]}, hour: {h}, number: {ToTiOcc_df.isnull().sum().sum()}')

            sparse_np = sparse.csr_matrix(np_df)
            actions_list_dfs.append(sparse_np)

        hours_list_lists.append(actions_list_dfs)

        if num_nans == 0:
            print(f'Good to go. No NaNs in array for hour {h}')
        else:
            print(f'Check array for hour: {h} :( Nans detected')

    final_np = np.asarray(hours_list_lists)
    print(f'transition matrix creation complete.')# Final numpy array with {final_df.ndim} dimensions: {len(final_df)}, {len(final_df[0])}, {len(final_df[0][0])} total size: {final_df.size}')
    
    return final_np

    # np.save('/Users/maggie/Desktop/TransitionMatrix_1.npy', final_df)                 # Save full output to a 4-dimensional numpy